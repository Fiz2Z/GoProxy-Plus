...
- **严格准入** — 出口 IP + 地理位置 + 延迟验证，HTTP 与 SOCKS5 都必须通过真实 HTTPS/TLS
- **自动优化** — 按需抓取（Emergency/Refill/Optimize 三模式），定时替换慢代理
- **故障自愈** — 首次失败冷却、连续失败禁用、定时复测恢复；免费代理持续失效 24–48 小时后才删除
- **质量优先轮换** — 随机端口按 S/A/B/C 质量加权，优先消耗 S/A 级代理，同时保留出口多样性
...
- 双角色权限：访客只读 + 管理员完全控制
- 中英文切换
- 应用租约、隔离代理，以及采集成功率、代理连接率、B站风控率三项独立统计
